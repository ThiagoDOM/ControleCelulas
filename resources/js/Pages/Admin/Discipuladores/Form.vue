...
const form = useForm({
    ...
    responsavel_id: user.value.role == 'pastor' ? user.value.id : usePage().props?.user.responsavel_id ?? '',
    password: '',
...
const updateUser = () => {
    form.patch(route('pastor.discipuladores.update', usePage().props.user.id), {
        preserveScroll: true,
        onSuccess: () => resetForm(),
        onError: () => {
...
const createUser = () => {
    form.post(route('pastor.discipuladores.store'), {
        preserveScroll: true,
...
const resetForm = (created = false) => {
    if (created)
        $toast.success("Discipulador criado com sucesso!", options);
    else
        $toast.success("Discipulador atualizado com sucesso!", options);
                        ...
                        <form @submit.prevent="user.id ? updateUser() : createUser()" class="mt-6 space-y-6">
                            ...
                            <div>
                                ...
                                <TextInput id="email" type="email" class="mt-1 block w-full" v-model="form.email"
                                    required autocomplete="username" placeholder="email@email.com" />
                            ...
                            <div>
                                ...
                                <TextInput id="telefone" type="text" class="mt-1 block w-full" v-model="form.telefone"
                                    autocomplete="telefone" placeholder="(##) #####-####" mask="(##) #####-####" />
